# Welcome to Ecksi's Bash Profile

To fully utilize this profile add the templates folder to your machine (be aware some of these files are hidden files and may not show up after you download them `ls -a` to see them in terminal or `cmd + shift + .` to show hidden files in finder). Then replace all instances of `~/turing/frontend/templates/` with the path to your templates folder.

***

To enjoy portions of this profile copy and paste the sections you would like to use into your bash profile.

To access your bash_profile open `~/.bash_profile` with your code editor of choice.
...
# creates react app with sass, eslint, enzyme, and remove bloat
crap() {
  create-react-app "$1"
  cd "$1"
...
# creates react app with router, sass, eslint, enzyme, and remove bloat - (This routerIndex.js for this alias is incomplete, it will be updated in the future)
crapr() {
  create-react-app "$1"
  cd "$1"
  cp ~/turing/frontend/templates/package.json ./
